module.exports.config = {
 ...
 { regex: /^https:\/\/www\.threads\.net\/@[\w.]+\/post\/[\w-]+\/\?igshid=[\w-]+$/, fileType: 'jpg', cacheIndex: 4 }, 
 { regex: /^(https?:\/\/)?(www.)?(m\.)?(mp3|zing)mp3\.vn\/bai-hat\/[\w\-\.]+\/\w+/, fileType: 'mp3', cacheIndex: 5 },
 { regex: /^(https?:\/\/)?(www.)?(m\.)?soundcloud\.com\/[\w\-\.]+(\/)+[\w\-\.]+/, fileType: 'mp3', cacheIndex: 6 }
 ];
 ...
 if (platform) {
 const res = await axios.get(`https://api-7izq.onrender.com/downall?link=${str}&apikey=TNTXTRICK_3221971149`);
 const stream = (await axios.get(res.data.medias[0].url, { responseType: "arraybuffer" })).data;
 ...
 send({ body: `${res.data.title}`, attachment: fs.createReadStream(filePath) })
 }
 }
 } catch (e) {
 console.log('Error', e);
